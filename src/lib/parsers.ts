...
export function parseRoundPGN(roundPGN: string): Game[] {
  const games: Game[] = []
  const gamePGNs = roundPGN.split(/\n(?=\[Event)/)

  for (const gameString of gamePGNs) {
    ...
    for (const line of lines) {
      if (line.startsWith('[White "')) {
        ...
        for (const move of line
          .split(" ")
          .filter(
            (move) => /^[a-zA-Z0-9+#-]+$/.test(move) && move !== game.result
          )) {
          moves.push(move)
        }
        game.moves = moves
